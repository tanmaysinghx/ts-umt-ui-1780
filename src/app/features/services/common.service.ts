...
export class CommonService {
  ...
  verifyJWTToken(token: string): Observable<any> {
    const assetUrl = environment.apiGatewayService + '/trigger-workflow/WF1625E10004?apiEndpoint=/v2/api/auth/verify/verify-token';
  ...
  generateJWTTokenBasedOnRefreshToken(refreshToken: string): Observable<any> {
    const assetUrl = environment.apiGatewayService + '/trigger-workflow/WF1625E10005?apiEndpoint=/v2/api/auth/refresh-token';
    const email = localStorage.getItem('user-email');
